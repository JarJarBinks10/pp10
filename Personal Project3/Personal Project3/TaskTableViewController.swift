...
class TaskTableViewController: UITableViewController {
    ...
    func loadSampleTasks() {
        let now = NSDate()
        let task1 = Task(name: "Task1", startDate: now, dueDate: now)
        let task2 = Task(name: "Task2", startDate: now, dueDate: now)
        let task3 = Task(name: "Task3", startDate: now, dueDate: now)
